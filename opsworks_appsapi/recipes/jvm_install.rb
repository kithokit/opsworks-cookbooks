...
local_custom_pkg_file = value_for_platform_family(
  'debian' => "/tmp/custom_jvm#{::File.extname(node['opsworks_appsapi']['jvm_pkg']['custom_pkg_location_url_debian'])}",
  'rhel' => "/tmp/custom_jvm#{::File.extname(node['opsworks_appsapi']['jvm_pkg']['custom_pkg_location_url_rhel'])}"
)

package "remove old JVM package #{old_jvm_pkg_to_remove}" do
  package_name old_jvm_pkg_to_remove
  action :nothing
  only_if { node['opsworks_appsapi']['jvm_version'].to_i >= 7 }
end

# install OpenJDK in either case to satisfy package dependencies
package node['opsworks_appsapi']['jvm_pkg']['name'] do
  action :install
...
bash "update the Java alternatives" do
  ...
  code <<-EOC
    if [[ "#{pkg_file_extension}" = ".rpm" ]]; then
      ...
      JAVA_BIN_DIR="#{node['opsworks_appsapi']['jvm_pkg']['java_home_basedir']}/$PARTIAL_JAVA_BIN_DIR"
...
execute "extract #{local_custom_pkg_file} to #{node['opsworks_appsapi']['jvm_pkg']['java_home_basedir']}" do
  command "tar -xzf #{local_custom_pkg_file}"
  cwd node['opsworks_appsapi']['jvm_pkg']['java_home_basedir']
  action :nothing
  notifies :run, 'bash[update the Java alternatives]', :immediately
end

directory node['opsworks_appsapi']['jvm_pkg']['java_home_basedir'] do
  owner 'root'
  group 'root'
  mode 0755
  action :nothing
...
remote_file local_custom_pkg_file do
  source node['opsworks_appsapi']['jvm_pkg']["custom_pkg_location_url_#{node[:platform_family]}"]
  only_if { node['opsworks_appsapi']['jvm_pkg']['use_custom_pkg_location'] }
  if ::File.extname(local_custom_pkg_file) == '.rpm'
    notifies :install, "rpm_package[#{local_custom_pkg_file}]", :immediately
  else
    notifies :create, "directory[#{node['opsworks_appsapi']['jvm_pkg']['java_home_basedir']}]", :immediately
    notifies :run, "execute[extract #{local_custom_pkg_file} to #{node['opsworks_appsapi']['jvm_pkg']['java_home_basedir']}]", :immediately
  end
end

include_recipe 'opsworks_appsapi::tomcat_setup'
